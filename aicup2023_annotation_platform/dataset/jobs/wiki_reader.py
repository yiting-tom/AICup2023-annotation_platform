...
def display():
    while True:
        logger.debug(
            "Queue sizes {0} {1}".format(
                arq.redirect_queue.qsize(), arq.article_queue.qsize()
            )
        )
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--wiki_file", required=True, type=str, help="wikipedia file path to read"
    )
    parser.add_argument(
        "--redirects_file", required=True, type=str, help="redirects file path to write"
    )
    parser.add_argument(
        "--s3_bucket",
        required=True,
