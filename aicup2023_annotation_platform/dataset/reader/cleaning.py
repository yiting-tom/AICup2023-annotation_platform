...
from util.recursive_clean import (
    recursive_clean_ipa,
    recursive_clean_lang,
    recursive_clean_convert,
)
...
def simple_clean(text):
    ...
    text = recursive_clean_ipa(
        text,
        [
            "\{\{IPAc-(([a-zA-Z]{2}\|)+)(audio[\s]*=[\s]*.*.ogg\|?(ˌ|)?)?",
            "\{\{IPAc",
            "\{\{IPA-(([a-zA-Z]{2}\|)+)",
            "\{\{IPA",
        ],
