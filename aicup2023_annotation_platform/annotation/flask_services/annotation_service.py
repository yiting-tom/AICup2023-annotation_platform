        ...
        for key in annotation["claims"][anno_type].keys():
            for line in annotation["claims"][anno_type][key].split("\n"):
                ...
                item["user"] = (
                    ...
                    if flask.request.remote_user is not None
                    else "guest"
                )
                item["date"] = str(datetime.datetime.utcnow())
                item["date_unix"] = int(datetime.datetime.utcnow().timestamp())
    ...
    return jsonify(
        {
            ...
            "mutation": mutation_types[next_mut],
        }
    )
...
def submit_stats():
    ...
    item["user"] = (
        flask.request.remote_user if flask.request.remote_user is not None else "guest"
    )
...
def get_claim(claim_id):
    session = create_session()
    claim = (
        session.query(Claim).join(Sentence).filter(Claim.id == int(claim_id)).first()
    )
            ...
            if w["canonical_entity"].strip().lower() != entity.strip().lower():
                ret[w["canonical_entity"]] = "\n".join(
                    [
                        t[1] if len(t) > 1 else ""
                        for t in [
                            b.split("\t") for b in body[: len(body) - 1].split("\n")
                        ]
                    ]
                )
...
def get_sentence(sent_id):
    ...
    matching_items = client.query(
        ...
        ExpressionAttributeValues={":v1": {"N": str(sent_id)}},
    )
...
def post_labels(label_id):
    ...
    session = create_session()
    claim = (
        session.query(Claim).join(Sentence).filter(Claim.id == int(label_id)).first()
    )
            ...
            for doc in labels["sentences"][str(verdict)].keys():
                for line in labels["sentences"][str(verdict)][doc].keys():
                    if bool(labels["sentences"][str(verdict)][doc][line]):
                        session.add(
                            LineAnnotation(
                                page=doc, line_number=int(line), verdict_id=vdct.id
                            )
                        )
