...
def process_scan(items):
    for item in tqdm(items):
        ...
        created = (
            datetime.datetime.utcfromtimestamp(int(item["date_unix"]))
            if "date_unix" in item
            else None
        )
        ...
        if (
            ...
            .filter(Sentence.entity_id == entity)
            .filter(Sentence.text == stext)
            .count()
            == 0
        ):
        ...
        if session.query(Claim).filter(Claim.text == original).count() == 0:
            session.add(
                Claim(
                    ...
                    isOracle=False,
                    text=original,
                    timeTakenToAnnotate=time_taken_to_annotate,
                    created=created,
        ...
        if (
            session.query(ClaimMutationType)
            .filter(ClaimMutationType.name == mutation_type)
            .count()
            == 0
        ):
        ...
        if session.query(Claim).filter(Claim.uuid == uuid).count() == 0:
            ...
            session.add(
                Claim(
                    testing=testing,
                    text=modified,
                    mutation_type_id=mutation_type,
                    original_claim_id=orid,
                    timeTakenToAnnotate=time_taken_to_annotate,
                    sentence_id=sid,
                    uuid=uuid,
                    created=created,
    ...
    print(
        ...
        session.query(Sentence).count(),
        session.query(ClaimMutationType).count(),
        session.query(Claim).count(),
        session.query(Claim).count(),
    )
