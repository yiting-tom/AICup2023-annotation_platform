...
for item in subordinate_list:
    ...
    subordinate_day_evidence[item["user"]][item["Date"]][item["claim_id"]].add(
        item["annotator"]
    )
    subordinate_week_evidence[item["user"]][item["Week"]][item["claim_id"]].add(
        item["annotator"]
    )
...
def prec_rec(relevant, retrieved):
    precision = (
        len(relevant.intersection(retrieved)) / len(retrieved)
        if len(retrieved) > 0
        else 1
    )
    recall = (
        len(relevant.intersection(retrieved)) / len(relevant)
        if len(relevant) > 0
        else 1
    )
...
def evidence_missed_by_all(dataset):
    ...
    missed_sorted_keys = sorted(
        missed_claims.keys(), key=lambda k: len(missed_claims[k]), reverse=True
    )
    for key in missed_sorted_keys:
        ...
        report_html += "<a href=%s>%s</a>\n" % (
            claim_link,
            subordinate_evidence_texts[key],
        )
...
def evidence_missed_by_user(dataset, user_set):
    ...
    for user in user_set:
        ...
        report_html += (
            '<a href="#" onclick="show_hide(\'%s\')">%s (%d claims)</a>\n'
        ...
        for key in missed_sorted_keys:
            if len(missed_claims[key]) == 0:
                continue
            claim_link = (
                "https://fever-annotate.corp.amazon.com/#!/label-claims/%d" % key
            )
            report_html += "<a href=%s>%s</a>\n" % (
                claim_link,
                subordinate_evidence_texts[key],
...
def macro_pr_all_by_user(dataset, user_set):
    ...
    return (
        sum(p) / len(p) if len(p) > 0 else "-",
        sum(r) / len(r) if len(r) > 0 else "-",
    )
...
for d in subordinate_week_evidence_all.keys():
    p_by_week[d], r_by_week[d] = macro_pr_time_by_user(
        subordinate_week_evidence, users, d
    )
