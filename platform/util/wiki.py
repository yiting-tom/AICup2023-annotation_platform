...
def get_wiki_entry(name, redirects):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "NoSuchKey":
            if name[0].islower():
                ...
                try:
                    return get_wiki_entry(
                        recursive_redirect_lookup(redirects, redirects[name]), redirects
                    )
                except RecursionError:
                    logger.error(
                        "Couldn't resolve {0} from dictionary: recursive redirect loop".format(
                            name
                        )
                    )
                    return None
            ...
            logger.error(
                "Could not resolve {0} from dictionary because it doesnt exist".format(
                    name
                )
            )
            return None
